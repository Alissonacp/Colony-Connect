...
export default function LembreteInfoPage() {
  ...
  // --- Estados de Edição ---
  const [editTitulo, setEditTitulo] = useState(lembrete.titulo || "");
  const [editDescricao, setEditDescricao] = useState(lembrete.descricao || "");

  // Data (Calendário)
  const [date, setDate] = useState(new Date());
  const [editData, setEditData] = useState(lembrete.dataLembrete || "");
  const [showDatePicker, setShowDatePicker] = useState(false);

  // Dropdowns States
  const [editStatus, setEditStatus] = useState(lembrete.status || "Ativo");
  ...
  useEffect(() => {
    if (selectedApiario && selectedApiario.id !== lembrete.apiarioId) {
      setTextoColmeias("");
      setSelectedColmeias([]);
    }
    ...
    if (selectedApiario) {
      ...
      onValue(colmeiasRef, (snapshot) => {
        ...
        if (selectedApiario.id === lembrete.apiarioId && lembrete.colmeiasIds) {
          const preSelected = lista.filter((c) =>
            lembrete.colmeiasIds.includes(c.id)
          );
  ...
  const handleSave = async () => {
    ...
    try {
      const refDb = ref(db, `usuarios/${user.uid}/lembretes/${lembrete.id}`);
      await update(refDb, {
        titulo: editTitulo,
        descricao: editDescricao,
        dataLembrete: editData,
      ...
      lembrete.titulo = editTitulo;
      lembrete.descricao = editDescricao;
      lembrete.dataLembrete = editData;
      lembrete.colmeiasRelacionadas = textoColmeias;
      lembrete.status = editStatus;
      lembrete.prioridade = editPrioridade;
      lembrete.apiarioNome = selectedApiario ? selectedApiario.nome : null;
    } catch (error) {
    ...
    Alert.alert("Deletar", "Tem certeza?", [
      { text: "Cancelar", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await remove(
              ref(db, `usuarios/${user.uid}/lembretes/${lembrete.id}`)
            );
            navigation.goBack();
          } catch (e) {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.infoContainer}>
          ...
          {isEditing ? (
            ...
            <Text style={styles.value}>
              {selectedApiario
                ...
                : lembrete.apiarioNome || "Não informado"}
            </Text>
          )}

          {/* COLMEIAS */}
          <Text style={styles.label}>Colmeias:</Text>
          {isEditing ? (
            <TouchableOpacity
              style={[
                styles.selectButton,
            ...
            <Text style={styles.value}>
              {textoColmeias || lembrete.colmeiasRelacionadas || "-"}
            </Text>
          )}
          ...
          {isEditing ? (
            ...
            <Text style={styles.value}>{editTitulo}</Text>
          )}
          ...
          {isEditing ? (
            ...
            <Text style={styles.value}>{editStatus}</Text>
          )}
          ...
          {isEditing ? (
            ...
            >
              <Text style={styles.selectText}>{editPrioridade}</Text>
              <Text style={styles.arrowText}>▼</Text>
            </TouchableOpacity>
          ) : (
            <Text style={styles.value}>{editPrioridade}</Text>
          )}
          ...
          {isEditing ? (
            ...
            <Text style={styles.value}>{editDescricao}</Text>
          )}
        </View>
